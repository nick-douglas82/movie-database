...
<template>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full flex flex-col md:flex-row relative z-10">
      <div class="mt-auto">
        <div class="p-6 mt-64 backdrop-blur-md bg-black/20 w-2/3">
          <h2 class="text-4xl mt-4 md:mt-0 font-semibold text-white">{{ state.tv.name }}</h2>
          <div class="flex flex-wrap items-center text-sm text-white mt-2">
            <AverageRating :rating="state.tv.vote_average" />
            ...
            <div class="mt-4">
              ...
              <h3 v-if="state.tv.tagline" class="font-semibold text-lg text-white mt-2">{{ state.tv.tagline }}</h3>
  ...
  <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 w-full px-4 py-16" v-if="state.credits.cast.length > 0">
