...
<template>
  <div class="actor-info relative bg-center">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full flex flex-col md:flex-row relative z-10 mb-16">
      <div class="flex-none">
        <img :src="`https://image.tmdb.org/t/p/w300/${state.actor.profile_path}`" alt="profile image" class="w-76" />
      </div>
      <div class="md:ml-24">
        <h2 class="text-4xl mt-4 md:mt-0 font-semibold">{{ state.actor.name }}</h2>
        <div class="flex flex-wrap items-center text-gray-400 text-sm">
          <CakeIcon class="w-4" />
          <span class="ml-2"
            >{{ formatDate(state.actor.birthday || '') }} ({{ calculateAge(state.actor.birthday || '') }} years old) in
            {{ state.actor.place_of_birth }}</span
          >
        </div>

        <p class="text-gray-500 mt-8">{{ state.actor.biography }}</p>

        <h4 class="font-semibold mt-12">Known For</h4>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
          <div v-for="(actorCredit, index) in state.credits" class="mt-4">
